...
> **Historical record, not maintained.** This is a VIS test-suite summary from early 2026. It describes the project at that time and the code has changed since. What changed and when is summarised in [CHANGELOG.md](../../CHANGELOG.md); for current documentation see [the documentation index](../README.md).
>
...
> - VIS is 910 ms, not 640 ms: the 8 data bits (LSB first, 1100 Hz = 1, 1300 Hz = 0) include the parity bit as bit 7, so there is no separate parity tone. MMSSTV's 16-bit VIS for the MR/MP/ML modes takes 1150 ms. See [the encoder specification](../specs/encoder.md).
...
> - The current suite is described in [the VIS code table](../specs/vis-code-table.md).
