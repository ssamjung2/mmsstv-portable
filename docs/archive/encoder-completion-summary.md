...
> **Historical record, not maintained.** This is an encoder completion summary from January 30, 2026. It describes the project at that time and the code has changed since. What changed and when is summarised in [CHANGELOG.md](../../CHANGELOG.md); for current documentation see [the documentation index](../README.md).
>
> Corrections (2026-09-18, checked against the code, MMSSTV and the SSTV Handbook):
>
> - VIS is 910 ms, not 940 ms: the 8 data bits (LSB first, 1100 Hz = 1, 1300 Hz = 0) include the parity bit as bit 7, so there is no separate parity tone. MMSSTV's 16-bit VIS for the MR/MP/ML modes takes 1150 ms. See [the encoder specification](../specs/encoder.md).
> - "All phases complete" referred to the encoder. The library now also has a decoder; see [the decoder specification](../specs/decoder.md) and [decoder status](../specs/decoder-status.md).
