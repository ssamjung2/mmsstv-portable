...
*Accepts:* `cmake --preset dev` then one command produces `pocketsstvd` and
`pocketsstv`;
...
**A7 · Fixture manifest and regenerated corpora (M)** · tier 1
Put the assets to work, stop committing what we can generate, and replace the
recordings whose origin nobody recorded.

*Regeneration recipe:* a small generator draws a synthetic test panel — colour
patches, grey wedge, resolution bars, text — deterministically from code, so
the source image is ours and reproducible. The encoder produces the "tight"
recordings from it, and a wrapper synthesises the "padded" variants by
prepending silence and appending a tone, reproducing the property that makes
the current `m1`/`s1`/`r36` files valuable. References are written as lossless
PNG rather than JPEG, so golden comparisons stop inheriting JPEG slack. P5 and
P7 gain the WAVs they are missing, which restores the two skipped
`timing_correction` cases.
