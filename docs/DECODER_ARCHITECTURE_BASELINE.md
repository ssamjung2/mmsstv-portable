...
limitations are summarised in [DECODER_STATUS.md](DECODER_STATUS.md).
...
| --- | --- | --- |
| Band-pass FIR | `MakeFilter` BPF, `24 × fs / 11025` taps (104 at 48 kHz), att = 20 dB. att < 21 selects no Kaiser window (α = 0, rectangular), so the worst stop-band rejection is only about 25–28 dB (measured; see [FILTER_SPECIFICATIONS.md](FILTER_SPECIFICATIONS.md)), not 60 dB. HBPF low edge 1100 Hz because MMSSTV's `m_SyncRestart` defaults to 1. Group delay = taps / 2 (1.08 ms). | `CSSTVDEM::CalcBPF` ("wide" setting) |
...
`d13`. See [FREQUENCY_ANALYSIS.md](FREQUENCY_ANALYSIS.md).
...
| `sstv_decoder_enable_debug_wav(...)` | Writes the signal after the 2-tap average, after the BPF, after the AGC, and "final" (= AGC output × 2, not the limiter output). See [DEBUG_WAV_GUIDE.md](DEBUG_WAV_GUIDE.md). |
