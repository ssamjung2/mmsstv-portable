...
- `tests/test_hf_impairments.cpp` therefore uses empirical noise levels: it sweeps white-noise RMS **2000, 6000, 10000, 15000, 20000** (16-bit PCM units) against the input scaled by 0.5. These are chosen for realistic-sounding noise, not calibrated to S-units; the S7/S9 labels the program prints are nominal. See [the HF impairment test guide](../guide/hf-impairment-testing.md).
